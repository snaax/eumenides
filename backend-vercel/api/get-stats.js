...
module.exports = async (req, res) => {
  ...
  try {
    ...
    let currentStreak = 0;
    const sortedAllTime = [...allTimeResult.rows].sort((a, b) =>
      new Date(b.stat_date) - new Date(a.stat_date)
    );
    ...
    const weekdayWeekendTotals = result.rows.reduce((acc, row) => {
    ...
    const totalEmotions = totals.emotions.anger + totals.emotions.frustration +
                          totals.emotions.irritation + totals.emotions.neutral;
    const totalHours = totals.hourlyPattern['00-05'] + totals.hourlyPattern['06-11'] +
                       totals.hourlyPattern['12-17'] + totals.hourlyPattern['18-23'];

    // Emotion percentages
    const emotionPercentages = {
      anger: totalEmotions > 0 ? Math.round((totals.emotions.anger / totalEmotions) * 100) : 0,
      frustration: totalEmotions > 0 ? Math.round((totals.emotions.frustration / totalEmotions) * 100) : 0,
      irritation: totalEmotions > 0 ? Math.round((totals.emotions.irritation / totalEmotions) * 100) : 0,
      neutral: totalEmotions > 0 ? Math.round((totals.emotions.neutral / totalEmotions) * 100) : 0
    };

    // Platform percentages
    const platformPercentages = {
      twitter: totalPosts > 0 ? Math.round((totals.platforms.twitter / totalPosts) * 100) : 0,
      reddit: totalPosts > 0 ? Math.round((totals.platforms.reddit / totalPosts) * 100) : 0,
      facebook: totalPosts > 0 ? Math.round((totals.platforms.facebook / totalPosts) * 100) : 0,
      linkedin: totalPosts > 0 ? Math.round((totals.platforms.linkedin / totalPosts) * 100) : 0
    };

    // Hourly percentages
    const hourlyPercentages = {
      '00-05': totalHours > 0 ? Math.round((totals.hourlyPattern['00-05'] / totalHours) * 100) : 0,
      '06-11': totalHours > 0 ? Math.round((totals.hourlyPattern['06-11'] / totalHours) * 100) : 0,
      '12-17': totalHours > 0 ? Math.round((totals.hourlyPattern['12-17'] / totalHours) * 100) : 0,
      '18-23': totalHours > 0 ? Math.round((totals.hourlyPattern['18-23'] / totalHours) * 100) : 0
    };
    ...
    const angerIntensityScore = totalPosts > 0
      ? Math.round(((totals.emotions.anger * 3 + totals.emotions.frustration * 2 + totals.emotions.irritation * 1) / totalPosts) * 10) / 10
      : 0;
    ...
    const weekdayPercentage = totalPosts > 0 ? Math.round((weekdayWeekendTotals.weekday / totalPosts) * 100) : 0;
    const weekendPercentage = totalPosts > 0 ? Math.round((weekdayWeekendTotals.weekend / totalPosts) * 100) : 0;
    ...
    if (topPlatform[1] > 80) {
      const platformNames = { twitter: 'Twitter', reddit: 'Reddit', facebook: 'Facebook', linkedin: 'LinkedIn' };
      insights.push({
        type: 'platform',
        icon: '📱',
