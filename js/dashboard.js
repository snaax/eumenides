document.addEventListener('DOMContentLoaded', function() {
  ...
  function setupPeriodSelectors() {
    ...
    periodBtns.forEach((btn, index) => {
      btn.addEventListener('click', () => {
        ...
        // Set period
        switch(index) {
          case 0: currentPeriodDays = 1; break;      // Today
          case 1: currentPeriodDays = 7; break;      // Week
          case 2: currentPeriodDays = 30; break;     // Month
          case 3: currentPeriodDays = 365; break;    // All
        }
  ...
  async function loadDashboardData() {
    try {
      // Get user email
      const syncData = await chrome.storage.sync.get(['premiumEmail', 'premium']);
  ...
  function renderAllStats(data) {
    ...
    if (analytics.bestDay) {
      ...
      document.getElementById('bestDayDate').textContent = date.toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' });
    } else {
  ...
  function renderEmotionalActivityChart(dailyData) {
    ...
    const maxValue = Math.max(...chartData.map(d =>
      d.emotions.anger + d.emotions.frustration + d.emotions.irritation + d.emotions.neutral
    ), 1);
  ...
  function renderInsights(insights) {
    ...
    container.innerHTML = insights.map(insight => `
      <div class="insight-card">
        <h3>${insight.icon} ${insight.type === 'pattern' ? 'Pattern détecté' :
                              insight.type === 'emotion' ? 'Composition émotionnelle' :
                              insight.type === 'platform' ? 'Plateforme déclencheur' :
                              insight.type === 'improvement' ? 'Amélioration' :
                              insight.type === 'achievement' ? 'Achievement' : 'Insight'}</h3>
        <p>${insight.message}</p>
      </div>
    `).join('');
  }
  ...
  function loadLocalPostsList() {
    chrome.storage.local.get(['history'], (data) => {
      ...
      if (recentPosts.length === 0) {
        container.innerHTML = '<p style="opacity: 0.5; text-align: center; padding: 20px;">Aucun post intercepté pour le moment</p>';
        return;
      }
  ...
  function calculateLocalStats(history) {
    ...
    const timeSavedMinutes = history.reduce((sum, p) => sum + (p.timeSaved || 3), 0);
    ...
    return {
      ...
      emotionDistribution: {},
      emotionPercentages: {},
      avgTimePerPost: totalPosts > 0 ? Math.round(timeSavedMinutes / totalPosts) : 0,
      bestDay: null,
      calmDays: 0,
      activeDays: 0,
      weekdayPercentage: 0,
