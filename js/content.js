...
(function () {
  ...
  function safeLocalStorageSet(data, callback) {
    try {
      ...
      console.warn(
        "Extension context invalidated, cannot save to local storage",
      );
    }
  }
  ...
  function safeLocalStorageGet(keys, callback) {
    try {
      ...
      console.warn(
        "Extension context invalidated, cannot read from local storage",
      );
      callback({});
    }
  }
  ...
  let postsToday = 0;
  let isPremium = false;
  let aggressionDetectionEnabled = true;
  ...
  safeStorageGet(
    ...
    (data) => {
      ...
      aggressionDetectionEnabled = data.aggressionDetection !== false;
  ...
  function saveAllowedPost(content) {
    // Increment allowedPostsToday counter
    safeStorageGet(["allowedPostsToday"], (data) => {
      const allowed = (data.allowedPostsToday || 0) + 1;
      safeStorageSet({ allowedPostsToday: allowed });
    });
    ...
    safeLocalStorageGet(["allowedPosts"], (data) => {
      const allowedPosts = data.allowedPosts || [];
      allowedPosts.unshift(entry);
      if (allowedPosts.length > 50) allowedPosts.pop();
      safeLocalStorageSet({ allowedPosts });
    });
  }
      ...
      function (e) {
        ...
        if (aggressionDetectionEnabled) {
          ...
          if (!analysis.isAggressive) {
            console.log(
              "Eumenides: Post allowed (not aggressive). Score:",
              analysis.score,
              "Reasons:",
              analysis.reasons,
            );
          ...
          console.log(
            "Eumenides: Post intercepted (aggressive). Score:",
            analysis.score,
            "Reasons:",
            analysis.reasons,
          );
  ...
  function injectRedditComment(content, isPreview = false) {
    ...
    if (isPreview) {
      ...
      if (existingComment) {
        ...
        const authorElement =
          fakeComment.querySelector('[slot="author"]') ||
        ...
        const bodyElement =
          fakeComment.querySelector('[slot="comment"]') ||
        ...
        const timeElement =
          fakeComment.querySelector("time") ||
  ...
  function injectTweet(content, isPreview = false) {
    ...
    if (isPreview) {
      ...
      const existingTweet = document.querySelector(
      ...
      if (existingTweet) {
        ...
        const tweetText =
          fakeTweet.querySelector('[data-testid="tweetText"]') ||
  ...
  function clearPostContent() {
    ...
    switch (platform) {
      case "twitter":
        ...
        break;
      case "facebook":
        const fbInput = document.querySelector(
          '[contenteditable="true"][role="textbox"]',
        );
  ...
  function showPostInterceptedNotification() {
    ...
    const existing = document.getElementById(
      "eumenides-intercept-notification",
    );
    ...
    switch (platform) {
      ...
      case "reddit":
        ...
        if (redditTextarea) {
          ...
          const redditEditor = document.querySelector(
            '[contenteditable="true"][role="textbox"]',
          );
          if (redditEditor) {
      ...
      case "facebook":
        const fbEditor = document.querySelector(
          '[contenteditable="true"][role="textbox"]',
        );
        if (fbEditor) {
  ...
  function showLimitReached() {
    showSuccessNotification(
      safeGetMessage("limitReached", "Daily limit reached"),
    );
  }

  function showPremiumRequired() {
    showSuccessNotification(
      safeGetMessage("premiumRequired", "Premium required"),
    );
  }
  ...
  chrome.storage.onChanged.addListener((changes, namespace) => {
    if (namespace === "sync") {
      ...
      if (changes.aggressionDetection) {
        aggressionDetectionEnabled = changes.aggressionDetection.newValue;
        console.log(
          "Eumenides: Aggression detection",
          aggressionDetectionEnabled ? "enabled" : "disabled",
        );
      }
      if (changes.detectionSensitivity) {
        detectionSensitivity = changes.detectionSensitivity.newValue;
        console.log(
          "Eumenides: Detection sensitivity changed to",
          detectionSensitivity,
        );
      }
    }
